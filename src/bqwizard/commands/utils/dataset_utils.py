...
def create_view(
    ...
    if len(target_dataset_parts) == 2:
        ...
        raise click.BadParameter(
            f"Invalid target_dataset_ref format: {target_dataset_ref}"
        )
        ...
        if index < (len(qualified_datasets) - 1):
            ...
            try:
                ...
                for table_name in tables_in_current_dataset:
                    if (
                        tables_to_include is not None
                        and table_name not in tables_to_include
                    ):
                    ...
                    click.echo(
                        f"{source_table_ref_for_view} --> {next_dataset_fq}.{table_name}"
                    )
                    create_view(
                        client, source_table_ref_for_view, next_dataset_fq, table_name
                    )
...
def describe_dataset(client, dataset, project):
    ...
    if isinstance(dataset, str):
        ...
        if len(dataset_parts) == 2:
            ...
            click.echo(
                f"Invalid dataset string format: {dataset}. Expected 'dataset' or 'project.dataset'."
            )
            return
    else:
    ...
    try:
        ...
        click.echo(
            f"Could not fetch dataset {project_id_to_use}.{dataset_id_to_fetch}: {e}"
        )
        return
        ...
        for t in tables_in_dataset:
            try:
                table_ref = client.get_table(
                    client.dataset(t.dataset_id, project=project_id_to_use).table(
                        t.table_id
                    )
                )
                table_data.append(
